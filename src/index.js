...
export function renderMarkdown(report) {
  ...
  lines.push(
    ...
    `## Next Step`,
    '',
    `[${report.cta.label}](${ctaUrlFor(report.cta.url, 'report')})`,
    '',
    report.cta.description,
    '',
    '_Generated by llms-txt-validator. No MaxAEO API calls or hidden telemetry were used._',
    ''
  );
  return `${lines.join('\n')}\n`;
}
...
export function renderTerminal(report) {
  ...
  lines.push('', `Next: ${report.cta.label}`);
  lines.push(ctaUrlFor(report.cta.url, 'cli'));
...
function ctaUrlFor(url, medium) {
  if (url.includes('utm_medium=')) {
    return url.replace(/utm_medium=[^&]+/, `utm_medium=${medium}`);
  }

  const separator = url.includes('?') ? '&' : '?';
  return `${url}${separator}utm_medium=${medium}`;
}
